...
## presentation 演示版澄清（2026-08-19）

### 目标

- 在 `presentation` 分支制作对外演示版，用于简历展示
- 演示版可公开访问，不依赖内网 MinIO
- 数据源使用本机本地原始数据集，数据留在本机，不提交 54GB 进 git

### 数据源

- `/mnt/data/gr00t-finetune/datasets/double_linkerhand_qingdao_1/2/3`
- 共 287 条原始 episode（90/97/100），约 54GB
- 使用 `processed/episode_XXXX/cameras/*.mp4 + manifest.json + metadata.json + telemetry.npz`
- 不使用 leRobot 转换后的数据集

### 功能范围

- 保留核心闭环：登录/角色、工作台、数据总库、任务类型与派发、reviewer/manual QC、QC 历史/导出、数据集导出、BUG、申请重检/审批
- manual QC 的视频预览、telemetry 曲线、L3 评分和异常时间线真实可用
- 隐藏或禁用 MinIO 扫描入口，替换为本地数据导入/绑定
- AI 助手/Ollama 不纳入公网演示

### 边界

- 公网演示不得暴露内网 MinIO 地址、凭据或生产数据
- 不改变 `main` 正式版业务语义，只改 `presentation` 分支
- 演示版提供重置脚本，演示数据被修改后可恢复初始状态

### 基线

- 基线 ID：`BASELINE-PRESENTATION-001`
- 状态：active
- 详细范围、验收、约束与未决项见 [baseline.yaml](/home/tbl/Project/data_collect/software-演示/.project-log/requirements/baseline.yaml)
