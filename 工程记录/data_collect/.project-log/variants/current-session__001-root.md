...
- 已完成真实生产态部署收口：`software/deploy/.env` 已落地私有 `SECRET_KEY`、`POSTGRES_PASSWORD`、MinIO endpoint/credentials 与默认 bucket `yaocao`，当前 compose 生产栈可直接在本机启动并连接真实 MinIO 数据
- 已补齐 `software/deploy/README.txt` 迁移部署手册，覆盖 secret 生成、`.env` 写法、首次 bootstrap、已有 PostgreSQL 卷改密、真实 MinIO 验证与跨机器迁移步骤
- 已完成真实 MinIO 生产栈现场验收：数据库页面可读取真实 episode/批次/扫描任务；扫描接口确认为 `POST /api/database/scan`；`GET /api/database/scan` 不提供状态读取
- 当前浏览器层完整验收仍受本机 Firefox/Playwright headless 启动异常阻塞，报错包含 `RenderCompositorSWGL failed mapping default framebuffer`，因此本轮对 database 页面主要采用“真实部署 + API 验证 + 页面源码复核 + 用户现场操作观察”的方式推进
- 已确认 `database` 页面原先顶部 `扫描入库` 与卡片内 `开始扫描` 为同一 `submitScan()` 动作的重复入口，现已删掉顶部重复按钮，仅保留扫描卡片中的单一主入口并统一文案为 `扫描入库`
- 已修复 `database` 页面 episode 列表区缺少独立纵向滚动的问题：当前表格容器已有独立滚动壳层，长列表浏览不再依赖整页滚动
