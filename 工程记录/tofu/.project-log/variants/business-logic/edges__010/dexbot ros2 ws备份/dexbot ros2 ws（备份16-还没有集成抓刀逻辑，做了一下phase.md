...
### Jump Mode (manual /phase_jump, skip_return_motion=True)

```yaml
edge_id: edge_4_or_6_jump_reprepare
from: PHASE_4_ROTATE_TOFU or PHASE_6_ROTATE_TOFU (via /phase_jump)
to: PHASE_2_MOVE_TO_PREPARE (re-entry)
path: main
status: stable
...
execution_chain:
  ...
  - 2. PhaseManager._set_phase() sets ctx.skip_return_motion=True
...
parameters: skip_return_motion (bool, set by /phase_jump handler)
...
notes: Bypasses knife_cut_action_server entirely. No arm motion. User must manually rotate tofu before touching continue file.
```
...
execution_chain:
  ...
  - 2. _execute_phase7_cut():
       ...
       e. Mid-push (cycles>1, at cut depth, before retract):
          push_forward: Z+ @ push_forward_speed → return → push_backward: Z- @ push_backward_speed → return
          Record mid_anchor position
       f. Executes seg2 (cut speed)
       g. Executes seg3 (cut speed)
       h. Tail-push (cycles>1):
          move to mid_anchor → cut down (Y-) → push_tail (Z- @ push_tail_speed) → retract (Y+)
...
notes: Cut direction HARDCODED base Y-. Step HARDCODED base Z. Push speeds independent.
```
