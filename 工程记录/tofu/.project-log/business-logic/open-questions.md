...
### Q-MIG-002: _BUNDLED_XCORE_SDK_ROOT Path Resolution

- Related branch: phase1-grab-migration
- Question: Classmate's code has `_BUNDLED_XCORE_SDK_ROOT = os.path.join(_FOLLOW_NODE_DIR, "../../../../dexbot_bottom_layer/...")` which resolves to a path missing `src/`. The code has fallback logic (`DEFAULT_XCORE_SDK_ROOT`, `LEGACY_XCORE_SDK_ROOT`, env var `DEXBOT_XCORE_SDK_ROOT`). Will the fallback work in our workspace?
- Why it matters: If SDK fails to load, xCore NRT direct connection fails → no arm control
- Options: (a) Trust fallback logic; (b) Fix path in migrated code; (c) Set env var before launch
- Current status: Open — to verify at runtime
- Answer: Unknown

### Q-MIG-004: O6 CAN Interface Configuration

- Related branch: phase1-grab-migration
- Question: Classmate's code uses `can0` for O6 gripper CAN communication. Is `can0` configured and up on the demo machine?
- Why it matters: If `can0` is not available, O6 gripper control fails → knife not grasped
- Options: (a) Confirm `can0` is configured; (b) Add setup script to launch file; (c) Mock O6 for testing
- Current status: Open — to confirm before demo
- Answer: Unknown
...
### Q-OBB-001: Multi-Frame Depth Median Location

- Related branch: feature-constrained-obb-vision
- Question: Should multi-frame depth median aggregation happen in `pose_estimator_node` (per-frame) or in `tofu_state_node` (sliding window)? Current proposal: per-frame depth may be sufficient; median across 20-50 frames could be done externally.
- Why it matters: Affects latency (median delay), pipeline design, and resource allocation. If in pose_estimator, each frame waits for N frames to accumulate, increasing latency by ~2-5 seconds (at 10Hz). If in tofu_state_node, the aggregation happens after pose estimation, which might be too late.
- Options: (a) Implement in pose_estimator as a rolling buffer; (b) implement as a separate pre-processing node; (c) skip initially, add only if needed based on quality metrics.
- Current status: Open
- Answer: Unknown

### Q-OBB-002: Background Subtraction Strategy

- Related branch: feature-constrained-obb-vision
- Question: If the scene contains a fixed stool or support, how to remove it from point cloud? Should we compute a static background depth map and subtract, or rely entirely on SAM3 mask quality?
- Why it matters: Background points pollute the OBB, causing oversized or shifted bounding box. Background subtraction is a common technique for fixed-camera setups.
- Options: (a) Implement empty-scene depth storage and runtime subtraction (stored as .npy); (b) improve SAM3 mask to exclude stool entirely; (c) rely on point cloud cleaning (ROR+DBSCAN) to filter background; (d) skip initially, observe quality first.
- Current status: Open
- Answer: Unknown

### Q-OBB-003: Fallback Behavior on OBB Failure

- Related branch: feature-constrained-obb-vision
- Question: When constrained OBB fitting fails (too few points after cleaning, degenerate geometry), what is the correct fallback?
- Why it matters: System must not crash; must provide a usable output for Phase2 prepare to continue operation.
- Options: (a) Automatically fall back to `corner_mode="aabb"` and continue; (b) return last valid OBB result with stale flag; (c) abort Phase2 with a clear error message.
- Current status: Open
- Answer: Unknown

### Q-OBB-004: Performance Budget for Target PC

- Related branch: feature-constrained-obb-vision
- Question: Is 200 ms per frame acceptable for pose estimation? The OBB search (360 iterations) plus cleaning may exceed this on low-end embedded PC.
- Why it matters: If it takes longer, the system may experience stale pose data, causing Phase2 to use old `top_corners`.
- Options: (a) Benchmark on target hardware; (b) increase `angle_step_deg` to 1.0° (180 iterations) if needed; (c) precompute trig tables; (d) profile and optimize hot path (e.g., use vectorized dot products for point projection).
- Current status: Open
- Answer: Unknown

## Resolved Questions

- ✅ Q-MIG-001: SAM3 prompt handling — **NO switching logic needed**. Phase2 launches a brand-new `sam3_detector_node` with `text_prompt="豆腐"` (hardcoded in `cuttofu_phase2.launch.py`) after classmate's lifecycle subprocess exits. Process isolation guarantees clean state: subprocess death = all classmate nodes die = no residual prompt state. Adding prompt switching would require modifying classmate's verified code, our SAM3 doesn't support multi-prompt syntax, and adds unnecessary settle time. Resolved 2026-05-19.
