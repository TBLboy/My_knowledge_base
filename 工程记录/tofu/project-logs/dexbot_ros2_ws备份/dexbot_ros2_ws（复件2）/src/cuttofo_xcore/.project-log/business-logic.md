...
> 创建日期: 2026-05-09
> 最后更新: 2026-05-13 (新增 Section 13: TCP Offset 标定与补偿)
...
| `knife_prepare_action_server` | **新建** | cuttofo_xcore | 🆕 | 刀预备位 Action Server |
| `tofu_cut_coordinator_node` | **新建** | cuttofo_xcore | 🆕 | 整体状态机协调 |
...
geometry_msgs/Vector3 edge_dir     # 豆腐左边棱边方向向量（归一化，在XZ平面内，与base_X+锐角）
...
- `edge_dir`: 由 `top_corners → A,B(Z最小2个) → v(AB方向,XZ投影,锐角约束) → 归一化` 得到
...
| `/arm_r/robot/move_rt_cartesian_segment` | Service | MoveRtCartesianSegment | RT 切削运动 |
...
def compute_edge_dir(corners_4, edge_align=True):
    """顶面4顶点 → 左边棱边方向向量（归一化，XZ平面内，与base_X+锐角）
...
class TofuCutCoordinator(Node):
    STATES = [
        "IDLE",               # 等待开始信号
        "WAITING_KNIFE",      # Phase 1: 等待拿刀信号
        "MOVING_TO_PREPARE",  # Phase 2: 移刀到预备位
        "CUTTING",            # Phase 3: 切豆腐 (RT循环)
        "WAITING_ROTATION",   # Phase 4: 等待旋转完毕
        "REPOSITIONING",      # Phase 5: 重新就位
        "HOMING",             # Phase 6: 归位
        "ERROR",              # 异常
    ]
    ...
    def run_state_machine(self):
        while self.state not in ("IDLE", "ERROR"):
            ...
            elif self.state == "WAITING_ROTATION":
                self._wait_for_tofu_rotated()
            elif self.state == "REPOSITIONING":
                self._move_to_prepare()  # 同 Phase 2 逻辑
            elif self.state == "HOMING":
                self._go_home()

    def _move_to_prepare(self):
        """Phase 2 / Phase 5: 调用 Action 移刀到预备位"""
    ...
    def _next_state_after_prepare(self):
        ...
        if self._is_first_prepare:
            return "CUTTING"           # Phase 2 → Phase 3
        else:
            return "CUTTING"           # Phase 5 → Phase 3
    ...
    def _execute_cutting(self):
        ...
        # 切削完成
        self.state = "WAITING_ROTATION"

    def _wait_for_tofu_rotated(self):
        """Phase 4: 等待 /tofu_rotated 信号"""
        # 阻塞等待 Bool topic
        # 收到 True → state = "REPOSITIONING"
        # 超时 → state = "ERROR" 或 "HOMING"
...
| l 方向约束 | — | l=cross(v,Y_up), l.z>0 | 自动满足（v.x>0 保证） |
| plane_angle max | 90° | **40-45°** | joint_6 ±40°安全限位 |
| 关节安全余量 | — | **15°** | IK求解硬约束 |
| A,B 选取 | — | Z最小的2个顶点 | 左边棱边端点 |
...
| v 方向约束 | v 在 XZ 平面内，v · base_X+ > 0 | v 与 base_X+ 保持锐角 |
| l 方向约束 | l ⊥ v，l 在 XZ 平面内，l · base_Z+ > 0 | l 与 base_Z+ 保持锐角（自动满足） |
...
A = top_corners[sorted_by_Z[0]]             ← Z 最小（最左侧）
B = top_corners[sorted_by_Z[1]]             ← Z 第二小（次左侧）

A, B 构成豆腐顶面的【左边棱边】的两个端点
```

几何含义：在 base 坐标系中，Z 越小越靠左。取 Z 最小的两个角点，
它们是顶面矩形左侧那条边的两个端点。
...
**定义**: v = 豆腐左边棱边 AB 的方向向量，投影到 XZ 平面，与 base_X+ 保持锐角。
...
**定义**: l = v 在 XZ 平面内的垂线方向，指向 arm 侧（+Z 方向）。

```
l = cross(v, [0, 1, 0]) = [-vz, 0, vx]
```

**性质验证**:
- l ⊥ v ✓（叉积定义保证正交）
- l 在 XZ 平面 ✓（Y 分量 = 0）
- l · base_Z+ = l.z = vx > 0 ✓（因为 v.x > 0 的锐角约束，l.z 自动为正）

**结论：不需要任何 flip 逻辑。v.x > 0 的约束自动保证 l.z > 0。**

**示例**:
| v | l = [-vz, 0, vx] | l.z |
|---|---|---|
| [1, 0, 0] | [0, 0, 1] | 1 > 0 ✓ |
| [0.9, 0, 0.4] | [-0.4, 0, 0.9] | 0.9 > 0 ✓ |
| [0.9, 0, -0.4] | [0.4, 0, 0.9] | 0.9 > 0 ✓ |
...
Step 2: A, B = Z 最小的 2 个顶点（左边棱边的两个端点）
        → sorted_idx = argsort(corners_4[:, 2])
        → A = corners_4[sorted_idx[0]], B = corners_4[sorted_idx[1]]
...
Step 4: l = cross(v, [0, 1, 0]) = [-vz, 0, vx]
        → 自动满足 l.z > 0（无需 flip）
...
Step 6: D' = D + offset_a * l
        → 沿 l 方向（向 arm 侧/+Z）偏移 offset_a

Step 7: TCP = [D'.x, top_y + vertical_offset, D'.z]
        → X, Z 在 D' 位置，Y 比顶面高 vertical_offset
```

**几何含义**: TCP 在豆腐左边棱边中点，向 arm 侧（+Z）偏移 3cm，抬高 3cm。
刀从 arm 侧接近豆腐左边棱边，悬停在顶面上方。
...
n = cross(v, Y_up) = [-ez, 0, ex]                    ← XZ 平面内垂直于 v（= l 方向）
tcp_Z = cos(α)·n - sin(α)·Y_up                       ← 刀面法线，从水平面倾斜 α 度
...
| **l** | [0, 0, 1]（固定 = base_Z+） | cross(v, Y_up)（跟随 v 变化） |
| **edge_dir** | [1, 0, 0] | 实际 AB 方向 |
| **刀脊 tcp_Y** | [1, 0, 0] = base_X | = v = 实际 AB 方向 |
| **TCP 偏移方向** | 纯 +Z（右） | 沿 l（⊥AB，指向 arm 侧） |
...
compute_edge_dir(corners_4)              compute_tcp_target_from_corners(corners_4)
  │                                        │
  │ Step 2: A,B = Z最小2个                  │ Step 2: A,B = Z最小2个
  │ Step 3: v = AB方向(XZ投影,锐角)         │ Step 3: v = AB方向(XZ投影,锐角)
  │ → edge_dir = v                         │ Step 4: l = cross(v, Y_up)
  │                                        │ Step 5: D = (A+B)/2
  │                                        │ Step 6: D' = D + offset_a * l
