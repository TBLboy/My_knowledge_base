...
- **Prepare mode**: Phase2/Phase6 re-prepare uses staged NRT joint-space motion, not RT Cartesian.
- **Staged prepare invariant**: First queued segment should complete most posture change before the last approach segment reaches tofu.
- **Staged prepare continuity**: The via segment and final segment must be queued into one SDK `moveAppend([...])` / `moveStart()` sequence to avoid dwell at the via point.
