...
- User's workspace calibration file path differs from classmate's: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/config/calibration_result.yaml` vs `/home/a/Desktop/dexbot_ros2_ws/src/config1/calibration_result.yaml`
...
## Vision Pose Estimation Parameters

| Parameter | Type | Default | Valid Range | Runtime Mapping | Used By | Requires Restart | Notes |
|---|---|---|---|---|---|---|---|
| `vision.corner_mode` | string | aabb | `aabb` / `pca_constrained` / `constrained_obb` | `cuttofu_phase2.launch.py`/`viz_display.launch.py` → `pose_estimator_node.corner_mode` | pose estimator | No | `constrained_obb` activates yaw-only OBB pipeline |
| `vision.y_filter_percentile` | float | 80.0 | 0.0-100.0 | launch → `pose_estimator_node.y_filter_percentile` → `get_pose_from_mask()` | pose estimator | No | Legacy top-surface filter for AABB/PCA path |
| `vision.x_percentile_low` | float | 0.01 | 0.0-100.0 | launch → `pose_estimator_node.x_percentile_low` → `get_pose_from_mask()` | pose estimator | No | Legacy AABB/PCA X lower bound |
| `vision.x_percentile_high` | float | 99.9 | 0.0-100.0 | launch → `pose_estimator_node.x_percentile_high` → `get_pose_from_mask()` | pose estimator | No | Legacy AABB/PCA X upper bound |
| `vision.z_percentile_low` | float | 0.01 | 0.0-100.0 | launch → `pose_estimator_node.z_percentile_low` → `get_pose_from_mask()` | pose estimator | No | Legacy AABB/PCA Z lower bound |
| `vision.z_percentile_high` | float | 99.9 | 0.0-100.0 | launch → `pose_estimator_node.z_percentile_high` → `get_pose_from_mask()` | pose estimator | No | Legacy AABB/PCA Z upper bound |
| `vision.top_y_percentile` | float | 99.9 | 0.0-100.0 | launch → `pose_estimator_node.top_y_percentile` → `get_pose_from_mask()` | pose estimator | No | Legacy top surface height estimate |
| `vision.mask_erode_px` | int | 5 | ≥0 | launch → `pose_estimator_node.mask_erode_px` → `get_pose_constrained_obb()` | pose estimator | No | Shrinks mask inward before back-projection |
| `vision.obb_margin` | float | 0.003 | ≥0.0 | launch → `pose_estimator_node.obb_margin` → `fit_constrained_obb_xz()` | pose estimator | No | Symmetric margin added to all OBB extents |
| `vision.obb_angle_step_deg` | float | 0.5 | >0.0 | launch → `pose_estimator_node.obb_angle_step_deg` → `fit_constrained_obb_xz()` | pose estimator | No | Yaw exhaustive-search resolution |
| `vision.obb_voxel_size` | float | 0.002 | ≥0.0 | launch → `pose_estimator_node.obb_voxel_size` → `clean_point_cloud()` | pose estimator | No | Voxel downsample size in meters |
| `vision.obb_sor_nb_neighbors` | int | 20 | ≥1 | launch → `pose_estimator_node.obb_sor_nb_neighbors` → `clean_point_cloud()` | pose estimator | No | Neighbor count for statistical outlier removal |
| `vision.obb_sor_std_ratio` | float | 1.0 | >0.0 | launch → `pose_estimator_node.obb_sor_std_ratio` → `clean_point_cloud()` | pose estimator | No | Std threshold for statistical outlier removal |
| `vision.obb_ror_radius` | float | 0.01 | >0.0 | launch → `pose_estimator_node.obb_ror_radius` → `clean_point_cloud()` | pose estimator | No | Radius threshold for radius outlier removal |
| `vision.obb_ror_nb_points` | int | 8 | ≥1 | launch → `pose_estimator_node.obb_ror_nb_points` → `clean_point_cloud()` | pose estimator | No | Min neighbors within radius |
| `vision.obb_dbscan_eps` | float | 0.01 | >0.0 | launch → `pose_estimator_node.obb_dbscan_eps` → `clean_point_cloud()` | pose estimator | No | DBSCAN cluster radius |
| `vision.obb_dbscan_min_points` | int | 10 | ≥1 | launch → `pose_estimator_node.obb_dbscan_min_points` → `clean_point_cloud()` | pose estimator | No | DBSCAN minimum cluster/core size |
| `vision.obb_depth_decimate_factor` | int | 1 | ≥1 | launch → `pose_estimator_node.obb_depth_decimate_factor` → `preprocess_depth_for_obb()` | pose estimator | No | Optional depth decimation proxy before smoothing |
| `vision.obb_depth_spatial_filter_d` | int | 5 | ≥1 | launch → `pose_estimator_node.obb_depth_spatial_filter_d` → `preprocess_depth_for_obb()` | pose estimator | No | Bilateral filter kernel diameter used as spatial filter proxy |
| `vision.obb_depth_spatial_sigma_color` | float | 20.0 | ≥0.0 | launch → `pose_estimator_node.obb_depth_spatial_sigma_color` → `preprocess_depth_for_obb()` | pose estimator | No | Bilateral sigmaColor |
| `vision.obb_depth_spatial_sigma_space` | float | 20.0 | ≥0.0 | launch → `pose_estimator_node.obb_depth_spatial_sigma_space` → `preprocess_depth_for_obb()` | pose estimator | No | Bilateral sigmaSpace |
| `vision.obb_depth_hole_fill_kernel` | int | 3 | ≥0 | launch → `pose_estimator_node.obb_depth_hole_fill_kernel` → `preprocess_depth_for_obb()` | pose estimator | No | Morphological hole-fill kernel size |
| `vision.obb_depth_median_frames` | int | 1 | ≥1 | launch → `pose_estimator_node.obb_depth_median_frames` → internal synchronized depth buffer | pose estimator | No | Median stack window for OBB path |
| `vision.obb_bounds_top_keep_ratio` | float | 0.8 | 0.01-1.0 | launch → `pose_estimator_node.obb_bounds_top_keep_ratio` → `fit_constrained_obb_xz()` | pose estimator | No | Keep highest ratio of bounds points by base Y for top-footprint OBB |
| `vision.obb_top_y_percentile` | float | 99.0 | 50.0-100.0 | launch → `pose_estimator_node.obb_top_y_percentile` → `fit_constrained_obb_xz()` | pose estimator | No | Percentile for top surface height from retained bounds points |
| `vision.obb_bounds_u_percentile_low` | float | 2.0 | 0.0-100.0 | launch → `pose_estimator_node.obb_bounds_u_percentile_low` → `fit_constrained_obb_xz()` | pose estimator | No | U-axis lower percentile to resist outlier expansion |
| `vision.obb_bounds_u_percentile_high` | float | 98.0 | 0.0-100.0 | launch → `pose_estimator_node.obb_bounds_u_percentile_high` → `fit_constrained_obb_xz()` | pose estimator | No | U-axis upper percentile to resist outlier expansion |
| `vision.obb_bounds_v_percentile_low` | float | 2.0 | 0.0-100.0 | launch → `pose_estimator_node.obb_bounds_v_percentile_low` → `fit_constrained_obb_xz()` | pose estimator | No | V-axis lower percentile to resist outlier expansion |
| `vision.obb_bounds_v_percentile_high` | float | 98.0 | 0.0-100.0 | launch → `pose_estimator_node.obb_bounds_v_percentile_high` → `fit_constrained_obb_xz()` | pose estimator | No | V-axis upper percentile to resist outlier expansion |
