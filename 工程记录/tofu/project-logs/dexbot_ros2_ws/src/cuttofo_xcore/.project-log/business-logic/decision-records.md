...
## 2026-05-24 - Phase7 Fixed to Pure RT Position Mode (No Impedance)

- **Decision**: Phase7 (vertical cut) will always execute in RT Cartesian position mode. Impedance mode, impedance fallback, and config choices are removed.
- **Context**:
  - Phase7 uses `move_rt_cartesian_path` which internally calls `stop()` + `setOperateMode(automatic)` + `setPowerState(on)` on every invocation.
  - With ~15 separate sub-segment calls, the SDK power cycles ~15 times during Phase7 alone, causing excessive relay/contactor wear.
  - Impedance mode was not providing benefit for vertical cutting (free-space motion after initial cut).
  - The `force_rt_position` parameter was always set to `true` in config; impedance was dead code.
- **Implementation**:
  - Removed `_phase7_force_rt_position()` method and param.
  - Removed impedance loop from `_execute_callback` for Phase7 — directly calls `_execute_phase7_cut` with `use_impedance=False`.
  - Removed impedance retry fallback inside `_move_segment`.
  - Phase7 reads RT velocity params directly from config instead of `_rt_params()` wrapper.
  - Cleaned config: removed `prefer_rt_impedance`, `fallback_to_rt_position`, `stiffness`, `cut_direction`.
  - Merged sub-segments to reduce total SDK calls from ~15 to ~5.
- **Alternatives considered**:
  - Keep impedance mode — rejected: no observed benefit, adds complexity, `force_rt_position=true` was always active.
  - Keep config toggle — rejected: only one mode is used, toggle introduces branch testing cost.
  - Merge segment calls — **selected**: reduces power cycling without SDK changes.
- **Evidence / Verification**: Config cleaned; build and syntax check passed.
- **Impacted nodes**: PHASE_7_THIRD_CUT
- **Impacted edges**: edge_7_to_done
- **Status**: active
...
- **Path verification**: All 3 paths confirmed resolving correctly:
  - SDK: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/dexbot_bottom_layer/dexbot_bottom_layer/xcoresdk_python-v0.5.1.ar_12` ✅
  - Linkerbot: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/dexbot_bottom_layer/dexbot_bottom_layer/linkerbot-python-sdk/src` ✅
  - Calibration: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/config/calib_right/calibration_result_right.yaml` ✅
...
- **Target**: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/dexbot_interfaces/dexbot_interfaces_mid/msg/`
- **Changes made**:
...
- **Target**: `/home/tbl/Project/tofu/dexbot_ros2_ws/src/dexbot_bottom_layer/dexbot_bottom_layer/xcoresdk_python-v0.5.1.ar_12/`
