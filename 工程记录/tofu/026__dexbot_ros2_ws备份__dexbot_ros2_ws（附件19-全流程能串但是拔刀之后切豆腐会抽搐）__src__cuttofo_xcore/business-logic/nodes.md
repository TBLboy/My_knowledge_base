...
status: stable
state:
  - System initialized (two launch modes available)
  - Mode A (standalone): All ROS nodes alive, PhaseManager waits for /knife_grabbed
  - Mode B (collaboration): Only phase1_monitor_node alive, no camera/SAM3 resources used
...
verification:
  - PhaseManager logs "Entered PHASE_1_GRAB_KNIFE" (standalone mode)
  - phase1_monitor_node logs "Phase1 Monitor ready" (collaboration mode)
...
notes:
  ...
  - Two launch modes:
    - Standalone: ros2 launch cuttofu_phase2.launch.py (all nodes, waits for /knife_grabbed)
    - Collaboration: ros2 launch cuttofu_phase1_monitor.launch.py (monitor only, leaves camera/SAM3 free for classmate's program)
  - Collaboration mode: monitor detects /knife_grabbed → waits 2s for resource release → spawns Phase2 launch with start_phase:=PHASE_2_MOVE_TO_PREPARE
---
...
status: stable
state:
  - Two modes: normal (from Phase3) or manual jump (from any phase)
  - Normal mode: Arm returned to prepare anchor, at wait joint pose, waiting for user
  - Jump mode: skip_return_motion=True; only waiting for /tmp/cuttofo_phase4_continue
inputs:
  - Normal: None (waits for user signal via terminal input() or touch file)
  - Jump mode: /tmp/cuttofo_phase4_continue touch file
outputs:
  - Normal: Arm at wait joint pose; triggers Phase4→Phase2 re-entry on user confirmation
  - Jump mode: Triggers Phase4→Phase2 re-entry immediately on continue file detection
data_format:
  - phase4_return_to_prepare config (source_phase, wait_joint_positions, wait_joint_speed)
  - Return-to-prepare is single-pose Cartesian move (normal mode only)
  - Wait pose: move_to_joints(7 joints, speed=0.3) (normal mode only)
related_interfaces:
...
related_hardware:
  - AR5 7-DOF robot arm (moves to neutral wait pose, normal mode only)
verification:
  - Normal: PhaseManager logs "Phase4 ready: rotate tofu manually"
  - Jump: PhaseManager logs "Phase4 jump: waiting for continue file"
...
notes:
  ...
  - Manual jump via /phase_jump "PHASE_4_ROTATE_TOFU" sets skip_return_motion=True
  - Jump mode skips return-to-prepare motion + wait pose move, only polls continue file
  - Jump mode sets prepare_next_phase=PHASE_5_SECOND_CUT automatically
  - start_phase:=PHASE_4_ROTATE_TOFU also enters jump mode (set in __init__)
---
...
status: stable
state:
  - Two modes: normal (from Phase5) or manual jump (from any phase)
  - Normal mode: Arm returned to prepare anchor, at wait joint pose, waiting for user
  - Jump mode: skip_return_motion=True; only waiting for /tmp/cuttofo_phase6_continue
inputs:
  - Normal: None (user signal)
  - Jump mode: /tmp/cuttofo_phase6_continue touch file
outputs:
  - Normal: Triggers Phase6→Phase2 re-entry, which transitions to Phase7
  - Jump mode: Triggers Phase6→Phase2 re-entry immediately on continue file detection
data_format:
...
verification:
  - Normal: PhaseManager logs "Phase6 ready: rotate tofu"
  - Jump: PhaseManager logs "Phase6 jump: waiting for continue file"
notes:
  - Identical structure to Phase4, different config section
  - Sets prepare_next_phase = PHASE_7_THIRD_CUT on success
  - Manual jump via /phase_jump "PHASE_6_ROTATE_TOFU" sets skip_return_motion=True
  - Jump mode skips return-to-prepare motion + wait pose move, only polls continue file
  - Jump mode sets prepare_next_phase=PHASE_7_THIRD_CUT automatically
  - start_phase:=PHASE_6_ROTATE_TOFU also enters jump mode (set in __init__)
---
...
state:
  ...
  - Each cycle: cut(Y-)→retract(Y+)→step(Z-)
  - Mid push: cut → lift(Y+) → push forward/backward → drop → retract
  - Tail push: move to mid-anchor → cut → lift → push left → retract
inputs:
  - Current flange pose (via xCore get_state)
outputs:
  - Phase7 complete → DONE
data_format:
  - phase7_third_cut config (cycles, cut_move, step_z, push_lift_y, push_*)
  - Segmented execution: seg1→mid-push→seg2→seg3→tail-push
  - push_lift_y lifts knife along base Y+ before each push to reduce friction
related_interfaces:
...
verification:
  - Phase7 logs "PHASE_7_THIRD_CUT: cycles=N mid_ci=M last_ci=L waypoints=W"
  - Each segment logs waypoint count and velocity
  - Lift log: "Phase7 lift" at push_lift velocity
...
notes:
  ...
  - push_lift_y lifts knife along base Y+ before each push to reduce cutting resistance
