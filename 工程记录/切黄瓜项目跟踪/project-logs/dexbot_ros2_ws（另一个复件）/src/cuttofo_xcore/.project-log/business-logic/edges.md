...
path: main
status: stable
method: Send MoveToPreparePose action goal to knife_prepare_action_server; execute staged queued MoveAbsJ prepare motion
execution_chain:
  ...
  - 3. knife_prepare_action_server pipeline:
       ...
       g. Read current TCP/flange pose and build staged via pose:
          - via_progress = 0.70 along current_TCP → target_TCP segment
          - via orientation is interpolated toward target orientation so most posture change is completed by the via point
       h. Solve IK for via pose using current joints as seed; fallback to single-stage if via IK fails or staged motion disabled
       i. Select best final candidate → queue two joint targets in one SDK command sequence:
          - command 1: q_via
          - command 2: q_prepare
          Single moveAppend queue + single moveStart, so there is no dwell at the via point
...
parameters:
  ...
  staged_motion_enable: true / config
  staged_via_progress: 0.70 / config
  staged_orientation_slerp: 0.70 / config
  staged_zone_mm: 10 / config
...
notes: Phase2 is entered 3 times. tcp_offset handled by adapter layer internally. Staged prepare is used both on first entry and on Phase4/Phase6 re-entry because both flows share the same action server.
```
