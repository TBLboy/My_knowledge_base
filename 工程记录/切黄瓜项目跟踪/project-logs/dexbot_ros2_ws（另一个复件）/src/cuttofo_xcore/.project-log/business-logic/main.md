...
- **Phase2**: Vision-guided prepare — knife moves to tofu right edge (entered 3 times). Prepare motion uses a two-stage queued joint-space plan: a 70% via-point first, then final target, so posture is adjusted before the last approach segment.
...
- **Phase6**: Return to anchor, wait for second user rotation, re-prepare with vertical knife (phase6_prepare config). Same return_extra_offset_m safety margin as Phase4. Re-prepare uses the same two-stage queued joint-space plan as Phase2. Manual jump mode skips return motion, only waits for continue file
