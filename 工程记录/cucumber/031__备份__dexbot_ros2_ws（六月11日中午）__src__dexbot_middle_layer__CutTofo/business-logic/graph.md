...
- 分支约束：第二次横切新分支按 cycle 内闭环执行 `hook_lift -> transfer_to_container -> return_next_anchor -> next cut`，而不是整轮切完后再统一转运。
- 分支约束：容器位置可持续检测，但每一轮只在生成该轮 `transfer_to_container` 轨迹前读取一次目标；优先 latest，缺失时回退 cached，本轮轨迹生成后不在运动过程中重规划。
- TCP 约束：右臂该阶段工作 TCP = 刀刃中心；左臂该阶段工作 TCP = 灵巧手手指操作点。
- 控制约束：`hook_lift -> transfer_to_container -> return_next_anchor` 必须并入同一条 RT 连续轨迹一次性下发，不允许在转运到容器或回下一刀位之间插入独立控制请求。
