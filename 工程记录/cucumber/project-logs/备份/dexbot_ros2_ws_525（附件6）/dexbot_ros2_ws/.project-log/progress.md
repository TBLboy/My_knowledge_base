...
## 2026-05-31 18:20 Local Time — 左臂 hold press 根因定位 + IK 预检实现

- Objective: 定位左臂 hold press 段失败的根因，实现 IK 预检保险策略。
- Work completed:
  - **根因定位**: press 在固定 `target_flange_quat_xyzw` 约束下 IK 不可达（code=-32），不是 SDK 吞命令。approach 完成后构型变化，固定 pose 落在 IK 可达域外。
  - 当前控制方案（基于 `move_to_approach_with_orientation`）是正确的，但缺少 IK 预检导致先走 approach 再空跑 fail。
  - **实现 IK 预检机制**:
    - `xcore_direct_motion.py` — 新增 `resolve_tcp_goal_ik()`，可在不运动的前提下调用 FK + calcIk。
    - `xcore_direct_executor.py` — 新增 `precheck_tcp_fixed_orientation()`，失败输出中文可读提示（含 press 坐标、quat、SDK 错误、调整建议）。
    - `cucumber_hold_workflow.py` — approach 前调用 precheck，失败走 `ik_precheck` stage。
    - `tofu_codes.py` — 新增 `BIZ_CHOLD_IK_PRECHECK=3406`。
  - 更新 KI-004 记录：从「未解决」更新为「已定位根因 + 实现预检」。
- Business logic impact: 新增 cucumber_hold 的 approach → press 边上的 IK 预检步骤，减少无意义空跑。
- Problems encountered:
  - 最初尝试两段 `move_to_approach_with_orientation` + SDK reconnect 仍有 IK 不可达问题（黄瓜偏离时）。
  - `resolve_tcp_goal_ik` 的 `initial_joints` 参数在 SDK 层被忽略（`del arm, initial_joints`），不影响预检准确性。
- Resolution:
  - 实现了 IK 预检提前拦截不可达情况。预检失败会给出具体坐标与调整建议。
  - 预检通过才执行 approach → press 运动链。
- Verification:
  - `colcon build` 成功（cuttofo_skill_common + cuttofo_skill_cucumber_hold）。
  - 5 个 mock 单元测试通过（FK 失败 / IK 失败 / 成功 3 种场景 + executor precheck 成功/失败）。
  - 实机验证待执行。
- Unverified items:
  - 实机验证 IK 预检可拦截不可达 case（需重现黄瓜偏离场景）。
  - 实机验证预检通过后 press 可达。
- Files changed:
  - `cuttofo_skill_common/arm/xcore_direct_motion.py` — 新增 `resolve_tcp_goal_ik`，`move_tcp_fixed_orientation` 复用
  - `cuttofo_skill_common/arm/xcore_direct_executor.py` — 新增 `precheck_tcp_fixed_orientation`
  - `cuttofo_skill_cucumber_hold/cucumber_hold_workflow.py` — 接入 precheck，`ik_precheck` stage
  - `cuttofo_skill_common/errors/tofu_codes.py` — `BIZ_CHOLD_IK_PRECHECK=3406`
  - `cuttofo_skill_common/test/test_resolve_tcp_goal_ik.py` — 5 个 mock 单测
  - `.project-log/debugging/known-issues.md` — KI-004 更新根因与预检方案
- Next steps:
  - 等待用户开始右臂调试工作。
