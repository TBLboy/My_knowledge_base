...
| Joint polling | `after()` callback | 500ms interval joint state reading | `after(500, _poll_joints)` | Recurring, stops on window close |
...
5. **Joint polling**: Uses `after()` which runs on main thread — no threading conflict
