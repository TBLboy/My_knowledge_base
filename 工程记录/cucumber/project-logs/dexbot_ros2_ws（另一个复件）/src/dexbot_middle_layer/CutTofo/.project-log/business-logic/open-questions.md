...
- Current consensus:
  1. 容器检测直接消费现有 vision 节点输出，不新增独立感知节点；检测与提示词切换直接封装在 `second_cross_cut` skill 包内部：skill 启动时切到容器提示词，skill 结束时无论成功/失败/取消都切回豆腐提示词。
  2. 容器检测插入在第二次横切 skill 内部；当第二次横切开始时，豆腐位姿已不再是本 skill 的主要感知对象，因此可以切换到容器提示词并直接读取容器中心。
  3. 新第二次横切分支执行期间容器位置可持续检测，但每一轮只在生成该轮 `transfer_to_container` 轨迹前读取一次目标；优先基于 latest 容器结果执行，若某次未检测到新容器，则回退使用本轮 skill 内缓存的上一次有效容器位姿作为补充策略。
...
- Related node: second_cross_cut 左臂 handoff
- Related edge: transfer → left_handoff
- Question: 现有 `left_flange_pose_candidates.yaml` 与 `right_flange_target_offset` 是否在「同刀点、只变姿态」前提下有效？绝对 offset 重采后能否收敛到 <5 mm？
- Why it matters: 6 组历史 `left_actual` 相差 ~4–5 cm，mean offset 可能无法代表单一接触点。
- Current status: Open
- Answer: 待 `debug_left_handoff` 在 FLANGE:: + from_euler 修复后给出 `err=XX.Xmm`，并决定是否重采。
