...
## 2026-06-15 CST (Online IK 多候选求解 — 方案 C 实现与测试)

- Type: feature | performance optimization
- Status: code complete, compiled, pending real-robot test
- Importance: high
- Reusable: yes (motion_planner_node compute_ik 服务调用可复用)
- Objective: 解决 online IK (`move_cartesian`) 两大问题：下刀刮蹭豆腐 + 回等待位碰倒豆腐。

**问题根因分析：**
- **层面 1（直接原因）**：`move_cartesian` 笛卡尔直线运动穿过豆腐表面 → 需改用 `move_to_joints`（关节空间弧线）
- **层面 2（深层原因）**：单次 IK 解的关节构型可能肘/腕离豆腐太近 → 需多候选求解

**分阶段策略：**
- **Phase 1（已实现）**：方案 C — 单次 `compute_ik` + `move_to_joints` 执行，解决层面 1
- **Phase 2（条件触发）**：方案 A — 混合 FK + 多候选求解，如果 Phase 1 测试后层面 2 仍存在

**方案 C 实现：**
- 在线路径改用：`compute_ik` → `move_to_joints` → `verify_arrival` → TCP 验证
- 使用 `motion_planner_node` 的 `/dexbot/motion/compute_ik` 服务（单次 10-15s）
- 失败时自动 fallback 到离线 URDF IK（现有逻辑保留）

**Files changed (Phase 1):**
- `cuttofu_skill_common/arm/xcore_arm_adapter.py`：
  - 新增 `ComputeIK` 导入 + `_compute_ik_cli` 客户端
  - 新增 `compute_ik(target_pos, target_R, seed_joints)` 方法
- `cuttofu_skill_tofu_prepare/cuttofo_skill_tofu_prepare/tofu_prepare_workflow.py`：
  - 在线路径重构（lines 142-198）
  - 替换 `move_cartesian` 为 `compute_ik` + `move_to_joints`

**Compilation:**
- ✓ `colcon build --packages-select cuttofo_skill_tofu_prepare --symlink-install --paths ...` 成功（0.65s）

**Next steps:**
- 真机测试 `ik_mode: online`：验证关节空间弧线不刮豆腐
- 测试回等待位置是否仍碰倒豆腐 → 若碰倒则触发 Phase 2

**方案 A（备选，Phase 2）：**
- motion_planner_node 混合 FK（URDF FK 雅可比 + 控制器 FK 误差）
- 多候选求解 + preview rollout + scoring
- 期望性能：~1s/次 IK → 20 候选 ~20s
- URDF 仅用于雅可比（收敛方向），解精度由控制器 FK 保证

**计划文档：** `/home/tbl/.claude/plans/online-ik-urdf-ik-sharded-sparkle.md`

---

## 2026-06-15 CST (Prepare Online IK — move_cartesian 直接调用控制器 IK)

- Type: feature | refactor
- Status: complete, real-robot verified
- Importance: critical
- Reusable: yes (move_cartesian + target_is_flange 模式可复用于其他 skill)
- Objective: 使用控制器硬件标定运动学做 prepare 阶段 IK，消除 URDF 模型与真实机械臂的偏差。

**设计转变：**
- 放弃之前的 `motion_planner_node` + `WeightedIK` + `compute_ik` 服务方案（性能瓶颈 10-18s，且混合FK从未在代码中实现）
- 改用 `arm.move_cartesian(flange_pose, target_is_flange=True)` — 控制器内部用硬件标定运动学做 IK + 执行运动
- **零 URDF 依赖**，无网络延迟瓶颈，无性能优化需求

**架构：**
- 在线路径（默认）：Vision → target TCP pose → flange pose → `move_cartesian` → verify `get_tcp_pose()`
- 离线回退：在线失败时自动 fallback 到现有多候选 URDF IK + preview rollout + `move_to_joints`
- TCP offset 逻辑：`flange_pos = target_pos - target_R @ tcp_offset`（TCP姿态=法兰姿态，仅平移偏移）

**Work completed:**

1. **`tofu_prepare_params.yaml`**：
   - 新增 `ik_mode: online`（默认），可配置 `offline` 强制使用 URDF IK
   - 新增 `cartesian_speed: 0.2`、`cartesian_timeout_s: 120.0`、`online_position_tolerance_mm: 5.0`

2. **`tofu_prepare_workflow.py`**：
   - 在线路径（~55 行）：构造 flange Pose → `move_cartesian(target_is_flange=True)` → 验证位姿误差
   - 成功时直接 return（跳过离线逻辑）
   - 失败时 warn log + fallback 到离线 URDF IK（现有代码完全保留）
   - 新增 `from scipy.spatial.transform import Rotation as R` 导入

**编译问题修复：**
- `cuttofo_skill_tofu_prepare` 嵌套在 `src/dexbot_middle_layer/CutTofo/` 下，被顶层 `package.xml` 遮蔽
- 需使用 `colcon build --packages-select cuttofo_skill_tofu_prepare --symlink-install --paths src/dexbot_middle_layer/CutTofo/cuttofu_skills/cuttofo_skill_tofu_prepare`
- `--symlink-install` 确保 egg-link → symlink 到源码，修改即时生效
...
## 2026-06-15 CST (编译问题文档化 + 启动偶发崩溃)

- Type: documentation | known-issue
- Status: complete (文档化); unverified (偶发崩溃)
- Importance: low (文档化); unknown (崩溃)
- Objective: 将编译问题记录到项目日志，方便后续快速解决；记录启动偶发崩溃现象备用。

**编译问题文档化：**
- 添加到 `.project-log/debugging/known-issues.md`
- 条目："Colcon 编译嵌套包发现失败（已解决）"
- 内容覆盖：
  - 症状描述（colcon ignoring unknown package）
  - 根因分析（顶层 package.xml 阻止递归发现）
  - 诊断步骤（colcon list、目录追踪、install 检查）
  - 解决方案命令（完整 --paths 参数示例）
  - symlink-install 效果说明（代码修改即时生效）
  - 长期建议（移包或移除顶层 package.xml）
- 目的：后续遇到相同问题可直接查阅解决

**启动偶发崩溃（未解决）：**
- 症状：`tofu_prepare_node` 启动时立即 crash，exit code -11（SIGSEGV）
- 日志时间：2026-06-15 14:53 CST
- 可能原因：内存访问错误、底层库 crash、ROS node 初始化异常
- 用户判断：偶发情况，暂时不深入排查
- 状态：未解决，仅记录现象备用

**Files changed:**
- `.project-log/debugging/known-issues.md` — 新增编译问题条目
- `.project-log/current-session.md` — 更新当前会话进度
- `.project-log/progress.md` — 本条记录

**Next steps:**
- 若偶发崩溃频率增加，再深入排查底层原因
- 其他工作继续正常推进

## 2026-06-15 CST (motion_planner_node IK 性能优化 — 混合 FK 策略，纯 numpy FK)

- Type: performance | bugfix
- Status: code complete, pending real-robot verification
- Importance: critical
- Reusable: yes (WeightedIK 的 `fk_jacobian` 参数 + `A7FKPure` 可复用)
- Objective: 解决 `motion_planner_node` IK 求解耗时 10-18 秒/次的性能问题，使 online IK 可实际使用。

**问题现象：**
- `tofu_prepare` workflow 使用 online IK（真机控制器）时，每次 `ComputeIK` 服务调用耗时 10-18 秒
- 日志显示 `IK 求解: 成功, 脏时: 14050.05ms`，明显异常
- 所有 IK 调用超时，workflow 失败

**根因分析：**
1. `WeightedIK.solve()` 使用数值差分计算雅可比矩阵（中心差分）
2. 每个关节需要 2 次 FK（q+δ, q-δ），7 关节 × 2 = **14 次 FK/迭代**
3. 加上当前位姿 FK，每迭代 **15 次 FK 调用**
4. 默认 `MAX_ITERATIONS = 80`，最多 **1200 次 FK 调用**
5. 每次 FK 调用 `robot.compute_forward_kinematics()` 是**网络往返机器人控制器**，约 8-15ms
6. 1200 × 10ms ≈ **12 秒**

**修复方案：混合 FK 策略（纯 numpy 本地 FK，无 Pinocchio 依赖）**
- **真机 FK** 用于误差计算（保证精度）
- **纯 numpy FK** 用于雅可比计算（瞬时，无网络延迟，无外部依赖）
- 每迭代只需 1 次网络调用（误差计算），80 迭代 ≈ **0.8-1.2 秒**

**方案演变：**
1. 初版：`ArmKinetix`（Pinocchio）→ 因 `pin` 依赖 `numpy>=2` 与 `cv2`（numpy<2）冲突
2. 用户选择：移除 Pinocchio，用纯 numpy 从 URDF 实现 FK
3. 最终：`A7FKPure` — 从 URDF 解析运动学参数，纯 numpy 矩阵运算

**Work completed:**

1. **`weighted_ik.py`**（底层 IK 求解器）：
   - `__init__` 新增 `fk_jacobian: Callable = None` 参数
   - 若未提供，默认使用 `fk_func`（保持向后兼容）
   - `_compute_jacobian()` 改用 `self.fk_jacobian` 计算雅可比
   - `solve_fast()` 同样改用 `self.fk_jacobian`

2. **`pure_fk.py`**（新增）：
   - `A7FKPure` 类，从 URDF 解析的 A7 右臂运动学参数
   - 7 关节偏移 + 旋转轴 + Base 固定偏移
   - `rotation_matrix()` 使用 Rodrigues 公式
   - `forward_kinematics()` 返回 `[x, y, z, rx, ry, rz]`
   - 零外部依赖（仅 numpy + scipy.spatial.transform）

3. **`motion_planner_node.py`**：
   - 移除 `sys` import 和 `_import_arm_kinetix()` 函数
   - 改为 `from .lbot_catch.pure_fk import A7FKPure`
   - `_init_lbot_components()` 创建 `A7FKPure(side=self.arm_type)`
   - `WeightedIK(fk_func=fk_real, fk_jacobian=fk_local)` — 混合 FK
   - 无 fallback 分支（纯 numpy 始终可用）

**数学依据：**
- 雅可比只需**近似正确**即可让迭代收敛
- URDF FK 与真机 FK 的几何参数差异很小（通常 <1mm），不影响雅可比方向
- 误差由真机 FK 计算，驱动收敛方向正确
- 纯 numpy FK 是矩阵乘法，单次 <0.1ms
...
| FK 网络调用/迭代 | 15 | 1 |
| 总网络调用（80 迭代） | ~1200 | ~80 |
| IK 耗时 | 10-18s | 0.8-1.2s |
| 外部依赖 | 无 | 无（移除 Pinocchio） |

**Files changed:**
- `src/dexbot_bottom_layer/dexbot_bottom_layer/lbot_catch/weighted_ik.py`
- `src/dexbot_bottom_layer/dexbot_bottom_layer/lbot_catch/pure_fk.py`（新增）
- `src/dexbot_bottom_layer/dexbot_bottom_layer/motion_planner_node.py`

**Verification:**
- `colcon build --packages-select dexbot_bottom_layer` ✓
- `A7FKPure.forward_kinematics([0]*7)` 零位 FK 输出正常 ✓
- numpy 1.26.4 + cv2 4.5.4 兼容 ✓
- 真机 IK 验证：未执行（需用户测试）

**Unverified items:**
- 纯 numpy FK 与真机 FK 的 Euler 角约定一致性
- 实际收敛迭代次数是否正常
- 最终 IK 解精度是否满足 prepare 要求

**Next steps:**
1. 真机运行 tofu workflow，观察 IK 耗时日志是否降至秒级
2. 若效果稳定，回退 `xcore_arm_adapter.py` / `prepare_solver.py` 的临时超时参数（30s → 5s）
3. 考虑将 `solve_fast()` 作为默认 IK 方法进一步提速

## 2026-06-15 CST (motion_planner_node 集成到 tofu workflow launch)

- Type: feature | integration
- Status: complete
- Importance: medium
- Objective: 减少 startup 命令数量，用户无需手动启动 `motion_planner_node`。

**Work completed:**
- `tofu_workflow_execute_v2_no_approach.launch.py` 新增 `motion_planner_node` Node 定义
- 新增 `robot_ip` / `arm_type` LaunchArgument，默认值使用右臂配置
- `robot_ip` fallback 链：`$DEXBOT_ARM_R_ROBOT_IP` → `$DEXBOT_ROBOT_IP` → `192.168.2.161`
- `arm_type` 默认 `"right"`（prepare 针对右臂）

**Startup 前后对比：**
- 原：3 条命令（bringup + skills + motion_planner_node）
- 新：2 条命令（bringup + workflow launch）

**Files changed:**
- `src/dexbot_middle_layer/CutTofo/cuttofo_orchestrator/launch/tofu_workflow_execute_v2_no_approach.launch.py`

## 2026-06-15 CST (Prepare IK 双模式：online 真机 + offline URDF 回退)

- Type: feature | enhancement
- Status: code complete, pending real-robot verification
- Importance: high
- Reusable: yes
- Objective: prepare 阶段 IK 求解新增真机控制器路径，解决离线 URDF 与实际设备不一致导致的精度风险。

**背景：**
- prepare 原先用 `OfflineURDFKinematics` + scipy `least_squares` 求解 IK，依赖 URDF 与真机完全一致
- URDF 难以保证与实际设备几何参数完全匹配，存在精度隐患
- 底层已有 `/dexbot/motion/compute_ik` ROS 服务（`motion_planner_node` 提供，内部 `WeightedIK` 使用真机 FK 函数）

**方案选择（方案 A：ROS 服务）：**
- `XcoreArmAdapter` 已基于 ROS 服务通信，加 ComputeIK 客户端最自然
- `ComputeIK.srv` 已存在，`pick_place_action_server.py` 已有调用参考
- 与 prepare workflow 的 `MultiThreadedExecutor` 兼容
- 不侵入 SDK 直连层，与现有架构一致

**Work completed:**
1. **`xcore_arm_adapter.py`**：
   - 新增 `ComputeIK` 服务客户端（`/dexbot/motion/compute_ik`）
   - 新增 `compute_ik_available` 属性（lazy probe，首次访问 wait_for_service 1s，不可用时 warn 并降级）
   - 新增 `compute_ik(target_pos, target_R, seed_joints, position_tolerance, orientation_tolerance, timeout_s)` 方法
   - 内部做 `Rotation → geometry_msgs/Pose` 转换，调用服务并返回 7 元素关节向量

2. **`prepare_solver.py`**：
   - 新增 `solve_prepare_candidates_online(arm, kin, target_pos, target_R, current_joints, lower_safe, upper_safe, candidate_count, ...)` 函数
   - 种子策略与 offline 版本一致：当前关节 + Q_HOME + 关节中心 + 高斯扰动 + 均匀随机
   - 每次 `arm.compute_ik()` 调用使用不同种子，规避真机 IK 返回单一解的限制
   - 关节限位验证 + 去重（3° 阈值），返回 `(candidates, stats)`，stats 含 `ik_source: "online"` 标识

3. **`tofu_prepare_workflow.py`**：
   - `execute_prepare()` 读取 profile 级 `ik_mode`（默认 `online_then_offline`）
   - online 优先 → 无解自动回退 offline URDF
   - 三种模式：`online`（仅真机）、`offline`（仅 URDF，旧逻辑）、`online_then_offline`（推荐）
   - preview 容错：online IK 候选若 offline preview rollout 失败（因 URDF/真机模型偏差），用 `_make_minimal_score()` 直接评分接受
   - 失败信息中记录 `ik_mode / source / online_service` 状态，便于诊断

4. **`tofu_prepare_params.yaml`**：
   - 新增顶层 `ik_mode: online_then_offline`（注释说明三种模式语义）

5. **附带修复 `cut_tofu_phase3_lib.py`**：
   - `_read_mask_top_baseline` 的 `if not host.mask_top_valid():` 两分支此前完全相同（返回值一致），检查形同虚设
   - 修正为 mask 无效时返回 None，让 `_mask_top_changed` 正确跳过变化检测

**Business logic impact:**
- prepare IK 默认改用真机控制器模型，提高精度
- 服务不可用（motion_planner_node 未启动）时自动回退到原 URDF 逻辑，不影响现有部署
- preview rollout 仍用 offline URDF（用于关节限位验证），但其失败不再阻塞 online 候选
- fk_verify / verify_arrival 仍由真机闭环验证

**Problems encountered:**
1. 真机 IK 服务 `/dexbot/motion/compute_ik` 由 `motion_planner_node` 提供，需要它在 prepare 节点启动前已运行
2. 真 IK 单次只返回一个解，无法像 offline `least_squares` 那样做残差优化 → 用多种子策略规避
3. online IK 候选与 offline URDF preview 可能因模型偏差冲突 → 增加容错路径

**Resolution:**
- 用 lazy probe `compute_ik_available` 属性避免节点启动时硬等待服务
- 复用 offline 版本的种子生成策略，保证候选多样性
- online 候选绕过 preview 直接用 minimal score 评分（仅关节级 cost）

**Verification:**
- `ast.parse` × 3（xcore_arm_adapter.py, prepare_solver.py, tofu_prepare_workflow.py）✓
- `yaml.safe_load`（tofu_prepare_params.yaml）✓
- 真机 IK 验证：未执行（需 motion_planner_node 在线）

**Unverified items:**
- `/dexbot/motion/compute_ik` 在 skill bringup 时是否已稳定可用
- online vs offline IK 解在真机上的精度差异尚未量化对比
- preview 容错路径（online 候选跳过 preview）的安全性需真机确认

**Files changed:**
- `src/dexbot_middle_layer/CutTofo/cuttofu_skills/cuttofo_skill_common/cuttofo_skill_common/arm/xcore_arm_adapter.py`
- `src/dexbot_middle_layer/CutTofo/cuttofu_skills/cuttofo_skill_common/cuttofo_skill_common/kinematics/prepare_solver.py`
- `src/dexbot_middle_layer/CutTofo/cuttofu_skills/cuttofo_skill_tofu_prepare/cuttofo_skill_tofu_prepare/tofu_prepare_workflow.py`
- `src/dexbot_middle_layer/CutTofo/cuttofu_skills/cuttofo_skill_tofu_prepare/config/tofu_prepare_params.yaml`
- `src/dexbot_middle_layer/CutTofo/ros/cut_tofu_phase3_lib.py`
- `src/dexbot_middle_layer/CutTofo/.project-log/current-session.md`
- `src/dexbot_middle_layer/CutTofo/.project-log/progress.md`

**Next steps:**
1. 确保 `motion_planner_node` 在 skill bringup launch 中已启动
2. 真机运行 prepare，观察日志 `Prepare IK: trying online` + `Prepare IK online result`
3. 对比 online vs offline 解的 `fk_position_error_mm`，量化精度提升
4. 若效果稳定，考虑将 online IK 扩展到 cut_round / vertical_cut

## 2026-06-11 CST (动作编辑器 — 可行性分析与设计)

- Type: design | analysis
- Status: analysis complete, pending implementation
- Importance: high
- Objective: 评估拖动录制 → 增量轨迹提取 → RT 回放的完整方案可行性。

**需求概要：**
- 手动拖动机械臂末端做出拟人复杂动作（拨、翻、抹等）
- 脚本打开拖动+高频录制法兰/TCP 笛卡尔位姿
- 从录制数据中减去初始位姿，提取空间增量轨迹（无时间戳）
- RT 回放时：当前位姿 ⊕ 增量轨迹 → 绝对 waypoints → `move_rt_cartesian_path`
- 输出 YAML，支持从不同初始姿态复现相同动作效果

**可行性结论：完全可行，现有基础设施 ~60-70%。**

已有能力：

| 能力 | 文件 | 状态 |
|------|------|------|
| 拖动模式 enableDrag(cartesianSpace, freely) | `drag_hybrid_record.py:84-108` | 验证通过 |
| 读 TCP 位姿 (tcpPose_m 4x4) | `slice_cycle_*_drag_post_return.py:211-222` | 原生 SDK，~2-5ms |
| 读法兰位姿 get_flange_pose() | `xcore_arm_adapter.py:137` | ROS 服务，~10-20ms |
| RT 笛卡尔路径 move_rt_cartesian_path(waypoints) | `xcore_arm_adapter.py:210` | 验证通过 |
| 高频采样+关键帧筛点 | `drag_hybrid_record.py:52-80` | 50Hz，min_delta 筛点 |

待实现（3 个脚本，合计 ~300-400 行）：

| 模块 | 预估行数 | 说明 |
|------|----------|------|
| 采集脚本 | ~150 行 | 原生 SDK `posture(flangeInBase, ec)` 直读法兰 6D 位姿，交互：Enter 开拖动+1s缓冲→录制，q 退出自动关拖动 |
| 增量处理 | ~80 行 | 离线：绝对位姿 → 减初始位姿 → delta 轨迹 → 空间降采样 → YAML |
| 重放引擎 | ~100 行 | 读增量轨迹 + 当前位姿 → 绝对 waypoints → move_rt_cartesian_path |

**技术决策：**
- **坐标系**：法兰坐标系（用户确认）。录制时读 `robot.posture(flangeInBase, ec)` 返回 `[x,y,z,rx,ry,rz]` 6D 法兰位姿，回放时 `move_rt_cartesian_path` 也是法兰 waypoints。两端坐标系一致，无需 TCP↔法兰转换。
- **采集方式**：原生 SDK `posture(flangeInBase)`，不依赖 ROS。延迟 ~2-5ms，轻松支持 50-100Hz。
- **姿态增量**：delta_rot[i] = R[i] · R[0]⁻¹，重放时 new_rot[i] = delta_rot[i] · R(new_start)
- **位置增量**：简单向量减，delta_pos[i] = pos[i] - pos[0]
- **YAML 格式**：`{name, start_pose: {pos:[x,y,z], euler:[rx,ry,rz]}, waypoints: [{pos:[dx,dy,dz], euler:[drx,dry,drz]}, ...], spatial_min_step_m}`
- **脚本位置**：`src/dexbot_high_layer/dexbot_high_layer/slice_cucumber/`，复用现有 `sdk_support.py`

**已知限制：**
- 不同初始姿态重放时，关节构型可能不同，需注意自碰撞/关节限位
- 起始法兰方向和录制时差太多，动作语义会变形（如"向下拨"变"向侧面拨"）
- 快速拖动时采样频率不足会丢失高频细节

## 2026-06-11 CST (左臂 wait 位置修正 + 日志精简 + 交接候选检查)

- Type: fix | tuning
- Status: complete
- Importance: high
- Objective: 修复左臂返回 wait 位置错误，减少噪声日志，检查手交候选一致性。

**左臂 wait 位置修正：**

| 项目 | 旧值 | 新值 |
|------|------|------|
| left_wait_joint_positions (deg) | [45.2, 33.8, 0, 89.2, 30.5, 27, 38.1] | [19.7, 28.4, -11.9, 74.4, 7.2, 5.1, 11.0] |
| left_wait_joint_speed | — | 0.3 |

- 弧度值已写入 `tofu_second_cross_cut_params.yaml` 的 `human_wait.left_wait_joint_positions`
- 用户真机验证确认左臂可以回到准备位置

**噪声日志精简：**
- 删除 5 处冗余日志块（~60 行）：reorient params, transfer target, container state, left_wait_joints resolution, parallel return START/COMPLETE, feedback_cb call
- 保留 `_left_return()` 内部的关键诊断日志（START, move_to_joints result, error）

**_DEBUG_STOP_AFTER_TRANSFER 行为：**
- `True` 时 workflow 在 transfer reorient 后立即 return，跳过 left handoff 和并行回位
- 已确认这不是 bug，是调试开关的预期行为

**左臂交接候选选择一致性检查：**
- 仅从部分日志中获得 2 个数据点：cycle 1 → capture_1, cycle 7 → capture_1
- 两者都选 capture_1，但不排除其他 cycle 有不同选择的可能
- 完整 8 cycle 日志被截断，无法完全分析

**用户手动修改：**
- `left_handoff.tcp_offset` 从 `[0.12296, -0.17450, -0.12458]` 改为 `[0.11796, -0.17450, -0.12458]`（X 分量 -0.005）
- 已保留，未覆盖

- Files changed:
  - `tofu_second_cross_cut_params.yaml` — left_wait_joint_positions 更新
  - `tofu_second_cross_cut_workflow.py` — 日志精简 + _left_return 诊断日志
...
## 2026-06-11 CST (hook_lift IK failure investigation + full workspace rebuild)

- Type: investigation | build
- Status: partial
- Importance: critical
- Objective: 调查 hook_lift IK 失败原因（用户反映 Cursor 加入左手逻辑后开始失败）。
- Work completed:
  1. **git diff 分析 Cursor 修改**：确认 `tofu_second_cross_cut_workflow.py` 被大幅重写（850+ 行修改），执行模型从"单次 RT 路径"改为"per-cycle RT + transfer + reorient + left handoff + parallel return"。
  2. **全量清理编译**：`rm -rf build/ install/ log/` → 发现工作空间包含两套包：
     - `src/` 下 11 包（`dexbot_interfaces_low`、`dexbot_bottom_layer`、`dexbot_bringup` 等）
     - `src/dexbot_middle_layer/CutTofo/` 下 19 包
     - **需要两个 `--base-paths`**：`colcon build --base-paths src --base-paths src/dexbot_middle_layer/CutTofo`
  3. **残留节点清理**：`/tofu_cut_round_node`、`/tofu_second_cross_cut_node`、`/camera/camera` 已清理。
- Problems encountered:
  1. CutTofo 包不在 `src/` 直接下，`colcon build` 默认不发现，需每次指定 `--base-paths`。
  2. 最终方案：`colcon build --base-paths src --base-paths src/dexbot_middle_layer/CutTofo`，30 包一次性通过。
- Unverified items:
  - 尚未对比 git 前后版本的 hook_lift anchor/waypoint 生成逻辑，未确认 Cursor 修改是否意外影响 hook_lift IK。
  - hook_lift IK 失败问题**仍未解决**，需要后续继续排查。
- Next steps:
  1. 对比 git 前后版本 `_cycle_anchor` / `_build_hook_lift_waypoints` 等 hook_lift 关键函数。
  2. 若确认为 Cursor 修改引入的问题，回退或修复受影响代码段。
  3. 若确认是纯 IK 极限问题，放宽 `safety_margin_deg` / `hook_dy_m` / `hook_target_plane_angle_deg`。

## 2026-06-12 11:12 CST

- Type: refactor | config | verification
- Status: validated
- Importance: high
- Reusable: yes
- Objective: 左臂 handoff 改为 per-candidate `derived_right_flange_target_offset`，删除 params 全局均值 offset。
- Work completed:
  1. **运行时选优链路**：`select_left_handoff_flange_pose` 对每个候选单独算 `left_tcp_target`（用该候选 offset）→ IK → 最近邻选优。
  2. **API**：`compute_left_handoff_tcp_target(..., right_flange_offset=...)`；`LeftHandoffSelection.handoff_point_right`。
  3. **配置清理**：`tofu_second_cross_cut_params.yaml` 删除 `left_handoff.right_flange_target_offset`；`tofu_second_cross_cut_config.py` 删除 `right_flange_target_offset` 与 `left_handoff_offset_mean()`。
  4. **脚本/日志**：`capture_left_handoff_flange_pose.py` offset 校验与 `--list` 显示 `runtime_offset`；workflow/debug 日志读选中候选的 offset。
  5. **文档**：`启动指令.md` 更新（不再「复制 mean 到 params」）。
  6. **构建**：`colcon build --packages-select cuttofo_skill_tofu_second_cross_cut` 成功；`debug_left_handoff --dry-run` 通过。
- Business logic impact: main — 左臂 handoff offset 从全局参数改为 per-candidate 属性，选优时按候选各自 offset 计算。
- Problems encountered: 无（与 GUI 左臂读数 bug 独立；后者已在 dexbot_bottom_layer 修复）。
- Verification: build + dry-run 通过；此前真机 handoff 到位已验证（5 组候选 + 微调 offset）。
- Unverified items:
  - 完整 8 cycle workflow 联调（`_DEBUG_STOP_AFTER_TRANSFER` 仍可能为 True）。
- Files changed:
  - `left_flange_pose_candidates.py`, `left_handoff_pose.py`
  - `tofu_second_cross_cut_params.yaml`, `tofu_second_cross_cut_config.py`
  - `tofu_second_cross_cut_workflow.py`, `debug_left_handoff.py`, `capture_left_handoff_flange_pose.py`
  - `启动指令.md`
- Next steps:
  1. 关闭 `_DEBUG_STOP_AFTER_TRANSFER` 后跑完整 second_cross_cut workflow。
  2. 按需补充「同刀点、只变姿态」候选采集规范。

## 2026-06-11 CST (hook_lift IK failure 恢复时右臂回位修复 + DEBUG_STOP 打开)

- Type: fix | debug
- Status: complete
- Importance: medium
- Objective: 修复 hook_lift IK 失败时右臂未回到等待位置的 bug；打开 transfer 后 debug 停位开关。
- Work completed:
  1. **右臂回位修复**：原代码在 IK 失败后用 `_run_rt(arm, [matrix16_to_pose(base_anchor)], ...)` 让右臂回到 base_anchor。但 base_anchor 是通过 `flange_matrix_from_arm(arm)` 在循环开始时捕获的当前位置，且所有 waypoint 是积累完才一次性 RT 执行（L619），所以 IK 检查时右臂**还没有动过**——`_run_rt` 到当前位置等于空操作。修复为用 `arm.move_to_joints(wait_joints, speed=wait_speed)` 走到 `human_wait.wait_joint_positions`（复用 L729 现有模式），同时移除 `except Exception: pass` 改用正确日志。
  2. **DEBUG_STOP_AFTER_TRANSFER = True**：打开调试开关，右臂执行完第一轮 cut+hoook+transfer+reorient 后停在容器上方，便于观察停位精度。
- Files changed:
  - `tofu_second_cross_cut_workflow.py` L457-471：IK 失败回位改用 `arm.move_to_joints(human_wait)`
  - `tofu_second_cross_cut_workflow.py` L33：`_DEBUG_STOP_AFTER_TRANSFER = False → True`
- Rebuild: colcon build --base-paths src --base-paths src/dexbot_middle_layer/CutTofo --packages-select cuttofo_skill_tofu_second_cross_cut ✓
...
- Status: complete
- Importance: high
- Objective: 将 `motion_planner_node` 集成到 tofu workflow launch，使启动流程从三步回归为两步（无需手动 ros2 run motion_planner_node）。
- Work completed:
  1. **修改 launch 文件**：`tofu_workflow_execute_v2_no_approach.launch.py` 新增：
     - `robot_ip` launch argument（从 `$DEXBOT_ROBOT_IP` 环境变量读取，fallback 192.168.2.84）
     - `arm_type` launch argument（默认 left）
     - `motion_planner_node` 节点（package: dexbot_bottom_layer）
  2. **启动流程简化**：
     - 之前：① dual_xcore_controllers.launch.py ② ros2 run motion_planner_node ③ tofu_workflow_execute_v2_no_approach.launch.py
     - 现在：① dual_xcore_controllers.launch.py ② tofu_workflow_execute_v2_no_approach.launch.py
  3. **新增 launch 参数**：
     - `robot_ip:=<IP>` 覆盖机器人 IP（默认读环境变量）
     - `arm_type:=<left|right>` 覆盖臂类型（默认 left）
- Files changed:
  - `cuttofo_orchestrator/launch/tofu_workflow_execute_v2_no_approach.launch.py`
- Context: 配合之前的 online IK 功能（xcore_arm_adapter.py 的 compute_ik 服务调用 + prepare_solver.py 的 solve_prepare_candidates_online），完整支持在线 IK 计算。
- Verification: 待真机测试验证 motion_planner_node 正常启动且 `/dexbot/motion/compute_ik` 服务可用。
