...
### ISSUE-20260612-001: Left Arm Joint Readback Frozen in GUI

- **Symptom**: Left arm joint display stuck (~19.7°); `/arm_l/joint_states` published but positions stale.
- **Root Cause**: xcore controller `get_joint_readout_rad()` used stale UDP joint cache during drag; not a GUI polling bug.
- **Fix**: `query_joint_positions_for_readout()` in `lbot_robot_xcore.py`; controller rebuild + restart required.
- **Status**: Resolved 2026-06-12 (user verified on hardware).
