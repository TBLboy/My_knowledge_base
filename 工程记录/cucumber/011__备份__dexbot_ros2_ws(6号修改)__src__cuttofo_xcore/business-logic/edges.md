...
path: main
status: stable
method: Send MoveToPreparePose action goal to knife_prepare_action_server; execute single-step NRT MoveJ prepare motion
execution_chain:
  ...
  - 3. knife_prepare_action_server pipeline:
       ...
       c. Build target rotation:
        ...
        d. TCP→flange: flange_target = TCP_target - target_R @ tcp_offset
           - tcp_offset is pure translation only; TCP orientation == flange orientation
           - Therefore tcp_Z == flange_Z, and preview along tcp_Z matches the real oblique cut direction
       e. IK/FK backend: real xCore SDK kinematics (`XcoreSdkKinematics`), not URDF
          - FK uses SDK `model.calcFk()` through `XCoreLbotRobot.compute_forward_kinematics()`
          - Joint bounds use controller soft limits when available, otherwise xMate Er Pro fallback limits
       f. IK solve: candidate_count seeds + random perturbations → scipy least_squares over SDK FK
          Accepted: pos_err < 1e-4m, rot_err < 0.06deg, within safe_bounds (limit ± safety_margin_deg)
        g. Preview scoring: generate cut_preview_poses along tcp_Z/flange_Z, rollout IK chain with the same SDK FK backend
        ...
        h. Select best final candidate by lowest total score
        i. Execute single-step NRT `move_to_joints(q_prepare)`
        j. verify_arrival(q_prepare)
...
interfaces: /move_to_prepare_pose (Action), /tofu_state, /joint_states, xCore SDK model FK/soft-limit APIs, /arm_r/robot/move_joints
error_handling: Tofu timeout/IK failure → action aborted → Phase ERROR
verification: Log "Phase2 prepare pose reached: Success"
notes: Phase2 is entered 3 times. tcp_offset is pure translation with no TCP/flange orientation delta. Both first entry and re-entry use single-step NRT prepare through the same action server. The prepare search no longer depends on URDF FK/limits.
```
...
path: main
status: stable
method: Reverse accumulated step displacements + safety extra offset + move to wait pose + wait for user
execution_chain:
  ...
  - 2. knife_cut_action_server:
       ...
       c. Adds return_extra_offset_m along base Z+ (safety margin to avoid scraping tofu)
       d. Single-pose return-to-prepare waypoint → move_rt_cartesian_path
       e. move_to_joints(wait_joint_positions, speed=wait_joint_speed)
       f. Wait for user confirmation:
          - TTY: input("Rotate tofu, then press Enter...")
          - Non-TTY: poll touch file (/tmp/cuttofo_phase4/6_continue)
       g. On confirmation: set prepare_next_phase, transition to Phase2
...
parameters: source_phase, wait_joint_positions(7 floats), wait_joint_speed(0.3), return_extra_offset_m(0.04), wait_continue_file / config
...
notes: Phase4 and Phase6 share identical code; differ only by config + continue file. Stale file cleared on entry. return_extra_offset_m ensures knife clears tofu when returning from left to right.
```
