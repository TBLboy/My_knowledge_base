...
用户确认（2026-08-17）：数据转换、13D embodiment/归一化配置、低显存训练配置、30,000-step 训练、open-loop 评估与结果分析、GR00T 代码工程地图均由用户本人编写和完成；远端部署框架为组内已有，用户负责基于该框架完成部署准备/接入梳理，不把框架本身写成个人开发。
...
**框架边界：**远端部署框架为组内已有；用户本人负责工程地图、推理边界梳理和基于该框架的部署准备，不把框架开发写为个人贡献。
